...
const filePath = path.join(__dirname, 'frontend/src/pages/dashboard/[id].tsx');
const content = fs.readFileSync(filePath, 'utf8');
const lines = content.split('\n');

// Lines 248-458 (0-indexed) are the skills section (incl. trailing empty line before {/* Edit */})
const before = lines.slice(0, 248);
const after = lines.slice(459); // Start from {/* Edit */}
...
const newLines = [...before, ...newSkillsLines.split('\n').slice(0, -1), '', ...after];
const newContent = newLines.join('\n');
fs.writeFileSync(filePath, newContent, 'utf8');
console.log('Done. Lines before:', lines.length, 'Lines after:', newLines.length);
