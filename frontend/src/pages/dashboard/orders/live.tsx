...
interface SLAConfig {
  stages: { paid: SlaStage; processing: SlaStage; done: SlaStage; };
  refreshSec: number;
}
const DEFAULT_SLA: SLAConfig = {
  stages: {
    paid:       { yellowMin: 3,  redMin: 8  },
    processing: { yellowMin: 10, redMin: 20 },
    done:       { yellowMin: 3,  redMin: 7  },
  },
  refreshSec: 5,
};
...
function loadSLA(): SLAConfig {
  ...
  try {
    const raw = JSON.parse(localStorage.getItem('kds_sla') ?? '{}') as Record<string, unknown>;
    if ('yellowMin' in raw) {
      return {
        stages: {
          paid:       { yellowMin: Number(raw.yellowMin) || 3,  redMin: Number(raw.redMin) || 8  },
          processing: { ...DEFAULT_SLA.stages.processing },
          done:       { ...DEFAULT_SLA.stages.done },
        },
        refreshSec: Number(raw.refreshSec) || DEFAULT_SLA.refreshSec,
      };
    }
    if ('stages' in raw) return { ...DEFAULT_SLA, ...(raw as Partial<SLAConfig>), stages: { ...DEFAULT_SLA.stages, ...((raw as Partial<SLAConfig>).stages ?? {}) } };
    return DEFAULT_SLA;
...
function SLAModal({ config, onSave, onClose }: { config: SLAConfig; onSave: (c: SLAConfig) => void; onClose: () => void }) {
  ...
  const upStage = (stage: keyof SLAConfig['stages'], key: keyof SlaStage, v: number) =>
    setD(prev => ({ ...prev, stages: { ...prev.stages, [stage]: { ...prev.stages[stage], [key]: Math.max(1, v) } } }));

  const STAGE_META: { key: keyof SLAConfig['stages']; icon: string; label: string }[] = [
    { key: 'paid',       icon: '🔔', label: 'Novo (aguarda preparo)' },
    { key: 'processing', icon: '⚙️',  label: 'Em Preparação' },
    { key: 'done',       icon: '✅',        label: 'Pronto (aguarda entrega/retirada)' },
  ];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
      <div className="bg-gray-800 rounded-2xl w-full max-w-sm p-6 space-y-4 border border-gray-700 shadow-2xl my-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-white text-base">⚙ SLA por etapa</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-white text-2xl leading-none">×</button>
        </div>

        {STAGE_META.map(({ key, icon, label }) => (
          <div key={key} className="bg-gray-700/50 rounded-xl p-3 space-y-2">
            <p className="text-xs font-semibold text-gray-200">{icon} {label}</p>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] text-yellow-400 block mb-1">🟡 Amarelo após (min)</label>
                <input type="number" min="1" step="1"
                  className="w-full bg-gray-600 border border-gray-500 rounded-lg px-2.5 py-2 text-white text-sm"
                  value={d.stages[key].yellowMin}
                  onChange={e => upStage(key, 'yellowMin', parseInt(e.target.value) || 1)}
                />
              </div>
              <div>
                <label className="text-[10px] text-red-400 block mb-1">🔴 Vermelho após (min)</label>
                <input type="number" min="1" step="1"
                  className="w-full bg-gray-600 border border-gray-500 rounded-lg px-2.5 py-2 text-white text-sm"
                  value={d.stages[key].redMin}
                  onChange={e => upStage(key, 'redMin', parseInt(e.target.value) || 1)}
                />
              </div>
            </div>
          </div>
        ...
        <div>
          <label className="text-xs text-gray-400 block mb-1.5">🔄 Refresh a cada (seg)</label>
          <input type="number" min="1" step="1"
            className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2.5 text-white text-sm"
            value={d.refreshSec}
            onChange={e => setD(prev => ({ ...prev, refreshSec: Math.max(1, parseInt(e.target.value) || 1) }))}
          />
        </div>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700 gap-2 flex-wrap">
        <div className="flex items-center gap-3 flex-wrap min-w-0">
          ...
          <div className="hidden lg:flex items-center gap-2.5 text-[11px] text-gray-400">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-400" /> verde</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-yellow-400" /> amarelo</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> cr\u00edtico</span>
          </div>
        </div>
              ...
              <div key={col.key} className="flex flex-col border-r border-gray-700 last:border-r-0">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  ...
                  {colOrders.map(order => {
                    ...
                    const isFinished = col.key === 'sent';
                    const stageSla = (sla.stages as Record<string, SlaStage | undefined>)[col.key];
                    const colors = isFinished || !stageSla ? null : slaColors(mins, stageSla.yellowMin, stageSla.redMin);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {(isFinished || !stageSla) && <span className="text-xs text-gray-500">{timeAgo(refTime)}</span>}
                        </div>
                        {/* Description */}
                        ...
                        {order.status === 'done' ? (
                          <div className="grid grid-cols-2 gap-1.5">
                            <button
                              onClick={() => advanceStatus(order, 'delivery')}
                              disabled={isAdv}
                              className="py-2 rounded-lg text-xs font-semibold bg-purple-600 hover:bg-purple-500 text-white transition-all active:scale-95 disabled:opacity-50"
                            >
                              {isAdv ? '\u23f3' : '\ud83d\ude97 Entrega'}
                            </button>
                            <button
                              onClick={() => advanceStatus(order, 'pickup')}
                              disabled={isAdv}
                              className="py-2 rounded-lg text-xs font-semibold bg-green-700 hover:bg-green-600 text-white transition-all active:scale-95 disabled:opacity-50"
                            >
                              {isAdv ? '\u23f3' : '\ud83c\udfea Retirada'}
                            </button>
                          </div>
